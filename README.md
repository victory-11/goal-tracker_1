...
3. **Add Neon Database**
   ...
   - This automatically adds `DATABASE_URL` and `DIRECT_URL` environment variables
...
5. **Done!** Your app is now live with:
...
cp .env.example .env
# Edit .env with your Neon database URLs for local testing
...
| `DIRECT_URL` | Neon direct connection URL (for migrations) | Yes |

Both are automatically added when you create a Neon database in Vercel Storage.
